...
def ttt_mlp_scan_forward(
    ...
    W2_init = tl.load(W2_init_ptr + F_F_offset).to(tl.float32)
    ...
    for i in range(NC):
        if i % checkpoint_group_size == 0:
            ...
            tl.store(W2_checkpoints_ptr + W2_checkpoint_offset, W2_init)
            tl.store(b2_checkpoints_ptr + b2_checkpoint_offset, b2_init)
    ...
    tl.store(W2_last_ptr + F_F_offset, W2_init)
    tl.store(b2_last_ptr + F_offset, b2_init)
