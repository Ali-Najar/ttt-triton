...
def ttt_mlp_mini_batch_forward(
    ...
    X2 = gelu_tanh(Z1)
    Z2 = tl.dot(X2.to(mp_dtype), W2_init.to(mp_dtype)) + b2_init
    ...
    grad_l_wrt_Z1 = tl.dot(grad_l_wrt_Z2.to(mp_dtype), tl.trans(W2_init).to(mp_dtype)) * gelu_bwd(Z1)

    W2_last = W2_init - tl.dot(tl.trans(last_eta_mini_batch * X2).to(mp_dtype), grad_l_wrt_Z2.to(mp_dtype))
    ...
    X2_bar = gelu_tanh(Z1_bar)
    Z2_bar = tl.dot(X2_bar.to(mp_dtype), W2_last.to(mp_dtype)) + b2_last
...
def ttt_mlp_mini_batch_backward(
    ...
    grad_L_X2_bar = tl.dot(grad_L_Z2_bar.to(mp_dtype), tl.trans(W2_last).to(mp_dtype))
    grad_L_Z1_bar = grad_L_X2_bar * gelu_bwd(Z1_bar)
    
    grad_L_W2_last += tl.dot(tl.trans(X2_bar).to(mp_dtype), grad_L_Z2_bar.to(mp_dtype))
    grad_L_b2_last += tl.sum(grad_L_Z2_bar, axis=0)[None, :]
    grad_L_W1_last += tl.dot(tl.trans(XQ_mini_batch).to(mp_dtype), grad_L_Z1_bar.to(mp_dtype))
    grad_L_b1_last += tl.sum(grad_L_Z1_bar, axis=0)[None, :]
    ...
    grad_L_grad_l_wrt_Z2 = -(
        tl.dot((last_eta_mini_batch * X2).to(mp_dtype), grad_L_W2_last.to(mp_dtype))
    ) - (last_eta_mini_batch * grad_L_b2_last) + tl.dot((grad_L_grad_l_wrt_Z1 * gelu_bwd(Z1)).to(mp_dtype), W2_init.to(mp_dtype))
    ...
    grad_l_wrt_Z2_Last = tl.trans(tl.dot(grad_L_W2_last.to(mp_dtype), tl.trans(grad_l_wrt_Z2).to(mp_dtype)))

    grad_L_XK_mini_batch = -grad_l_wrt_Z1_Last * last_eta_mini_batch

    grad_L_last_eta_in_mini_batch = tl.sum(
        -(grad_l_wrt_Z2_Last * X2) - (grad_L_b2_last * grad_l_wrt_Z2)
    ...
    # Stage 2: LnFusedL2BWD
    grad_L_W2_init = tl.dot(tl.trans(grad_L_grad_l_wrt_Z1 * gelu_bwd(Z1)).to(mp_dtype), grad_l_wrt_Z2.to(mp_dtype))
    ...
    grad_L_X2 = (
        ...
        - tl.dot(grad_l_wrt_Z2.to(mp_dtype), tl.trans(grad_L_W2_last).to(mp_dtype)) * last_eta_mini_batch
    )
    ...
    grad_L_W2_init = grad_L_W2_last + tl.dot(tl.trans(X2.to(mp_dtype)), grad_L_Z2.to(mp_dtype)) + grad_L_W2_init
    grad_L_b2_init = grad_L_b2_last + (tl.sum(grad_L_Z2, axis=0))
    grad_L_W1_init = grad_L_W1_last + tl.dot(tl.trans(XK_mini_batch.to(mp_dtype)), grad_L_Z1.to(mp_dtype))
...
def ttt_mlp_scan_backward(
    ...
    grad_L_W2_last = tl.load(grad_L_W2_last_ptr + F_F_offset).to(tl.float32)
    ...
    for checkpoint_idx in range(K - 1, -1, -1):
        ...
        W2_init = tl.load(W2_checkpoints_ptr + W2_checkpoint_offset).to(tl.float32)
        b2_init = tl.load(b2_checkpoints_ptr + b2_checkpoint_offset).to(tl.float32)
        ...
        for mini_batch_idx_in_group in range(0, checkpoint_group_size):
            ...
            if mini_batch_idx < NC:
                ...
                tl.store(W2_init_group_ptr + G_F_F_offset, W2_init)
                tl.store(Z1_group_ptr + G_CS_F_offset, Z1)
                tl.store(Z1_bar_group_ptr + G_CS_F_offset, Z1_bar)
                tl.store(X2_group_ptr + G_CS_F_offset, X2)
                tl.store(X2_bar_group_ptr + G_CS_F_offset, X2_bar)
        ...
        for mini_batch_idx_in_group in range(checkpoint_group_size - 1, -1, -1):
            ...
            if mini_batch_idx < NC:
                ...
                W2_curr = tl.load(W2_init_group_ptr + G_F_F_offset).to(tl.float32)
                
                Z1 = tl.load(Z1_group_ptr + G_CS_F_offset).to(tl.float32)
                Z1_bar = tl.load(Z1_bar_group_ptr + G_CS_F_offset).to(tl.float32)
                X2 = tl.load(X2_group_ptr + G_CS_F_offset).to(tl.float32)
                X2_bar = tl.load(X2_bar_group_ptr + G_CS_F_offset).to(tl.float32)
    ...
    tl.store(grad_L_W2_init_ptr + F_F_offset, grad_L_W2_last)
    tl.store(grad_L_b2_init_ptr + F_offset, grad_L_b2_last)
